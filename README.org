...
- =elfeed-curate-ask-gptel= (=i= key) will first ask you to enter a prompt (see
  ...
  ﻿[[https://github.com/karthink/gptel/wiki/Defining-custom-gptel-commands][gptel-request API]]) with that text along with the link content. The response
  will be displayed in the ~*ask-gptel*~ buffer and added to the kill ring. See
  [[https://bobonmedicaldevicesoftware.com/blog/2025/10/27/elfeed-curate-update-two-new-features/?_gl=1*l7nn8w*_ga*MjAzNTIyNjIxNS4xNzQ1MjUyMTc3*_ga_B2ECNGHWZ6*czE3NzQyMTM5NTAkbzQ5JGcwJHQxNzc0MjEzOTUwJGo2MCRsMCRoMA..][Elfeed Curate Update: Two new features]] for details.

  The [[https://github.com/karthink/gptel][gptel]] backends and models are configured separately and can be changed as
  desired. Testing =ask-gptel= was mostly done with ~ChatGPT:gpt-5~. Other backends and
...
If you put point on a link in the entry buffer, the =j= key
(=elfeed-curate-get-link=) will automatically create a <text> entry and puts it in
the annotation buffer for you to edit. It also puts the org link version of the
link in the kill ring. Use prefix key (=C-u=) to only copy the org link to the
kill ring.
...
| =elfeed-curate-gptel-prompt--default=                                          | "Briefly summarize:"                                                                                                                          | Default ask-gptel prompt string.                                                                                                                              |
